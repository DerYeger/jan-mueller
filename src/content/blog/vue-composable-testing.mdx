...
openGraphImage: '/open-graph/vue-composable-testing.png'
---
...
describe('Testing Vue Composables', () => {
  ...
  it.fails('composables with plugins fail', async () => {
    function useQueryCounter() {
      ...
      return useQuery({ queryKey: ['counter'], queryFn: async () => Promise.resolve(42) })
    }
...
I copy-and-own the below `withComponentLifecycle` utility in each of my Vue projects.
...
describe('Testing Vue Composables', () => {
  ...
  it('composables with inject work', () => {
    ...
    const { result: { count } } = withComponentLifecycle(useInjectCounter, { provided: { count: 0 } })
    expect(count).toBe(0)
  })
})
```
